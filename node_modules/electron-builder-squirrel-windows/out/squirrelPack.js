...
let buildInstaller = exports.buildInstaller = (() => {
    var _ref = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (options, outputDirectory, outFileNames, packager, appOutDir, outDir, arch) {
        ...
        yield (_bluebirdLst2 || _load_bluebirdLst2()).default.all([(0, (_fs || _load_fs()).copyFile)(_path.join(options.vendorPath, "Update.exe"), appUpdate).then(function () {
            return packager.signAndEditResources(appUpdate, arch, outDir);
...
let releasify = (() => {
    var _ref3 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (options, nupkgPath, outputDirectory, packageName) {
        ...
        const out = (yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).exec)(process.platform === "win32" ? _path.join(options.vendorPath, "Update.com") : "mono", (0, (_electronBuilderUtil || _load_electronBuilderUtil()).prepareWindowsExecutableArgs)(args, _path.join(options.vendorPath, "Update-Mono.exe")), {
            maxBuffer: 4 * 1024000
        })).trim();
...
let msi = (() => {
    var _ref4 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (options, nupkgPath, setupPath, outputDirectory, outFile) {
        ...
        yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).exec)(process.platform === "win32" ? _path.join(options.vendorPath, "Update.com") : "mono", (0, (_electronBuilderUtil || _load_electronBuilderUtil()).prepareWindowsExecutableArgs)(args, _path.join(options.vendorPath, "Update-Mono.exe")));
        //noinspection SpellCheckingInspection
...
function syncReleases(outputDirectory, options) {
    (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)("Sync releases to build delta package");
    const args = (0, (_electronBuilderUtil || _load_electronBuilderUtil()).prepareWindowsExecutableArgs)(["-u", options.remoteReleases, "-r", outputDirectory], _path.join(options.vendorPath, "SyncReleases.exe"));
