...
class SquirrelWindowsTarget extends (_electronBuilder || _load_electronBuilder()).Target {
    ...
    build(appOutDir, arch) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)(`Building Squirrel.Windows for arch ${(_electronBuilder || _load_electronBuilder()).Arch[arch]}`);
            if (arch === (_electronBuilder || _load_electronBuilder()).Arch.ia32) {
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).warn)("For windows consider only distributing 64-bit or use nsis target, see https://github.com/electron-userland/electron-builder/issues/359#issuecomment-214851130");
            }
    ...
    computeEffectiveDistOptions() {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (_this2.options.remoteReleases === true) {
                const info = yield packager.info.repositoryInfo;
                if (info == null) {
                    (0, (_electronBuilderUtil || _load_electronBuilderUtil()).warn)("remoteReleases set to true, but cannot get repository info");
                } else {
                    options.remoteReleases = `https://github.com/${info.user}/${info.project}`;
                    (0, (_electronBuilderUtil || _load_electronBuilderUtil()).log)(`remoteReleases is set to ${options.remoteReleases}`);
                }
            }
            return options;
        })();
...
function checkConflictingOptions(options) {
    ...
    if ("noMsi" in options) {
        (0, (_electronBuilderUtil || _load_electronBuilderUtil()).warn)(`noMsi is deprecated, please specify as "msi": true if you want to create an MSI installer`);
